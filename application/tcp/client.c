...
//   fd = open("receive.txt", O_WRONLY|O_CREAT|O_TRUNC);
   ...
   fputs("Client : File receive start.\n", stdout);

   while((str_len = read(sd, message, BUF)) != 0)
   {
...
//      write(fd, message, str_len);
 ...
 //  close(fd);
   ...
   fputs("Client : File receiving is completed.\n", stdout);
   fputs("Client : Insert the message to server -> ", stdout);
